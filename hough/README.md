...
Requires the `hough_line` function.
...
| `rho` | Row vector (pixels) | — | **Output.** Rho axis values in pixels, symmetric around zero. Range is `[-ceil(D), ceil(D)]` where `D` is the image diagonal. |
...
### TC-03 — Horizontal Line Peak at theta=-90

Verifies that a horizontal line in the image produces the strongest peak at `theta = -90` degrees.
