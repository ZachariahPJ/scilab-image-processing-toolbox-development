...
// ------------------------------------------------------------
// TC-02: Horizontal line peak is at theta=-90
// ------------------------------------------------------------
...
[H, theta, rho] = hough(bw);
peaks = houghpeaks(H);
ok = (theta(peaks(1,2)) == -90);
report("Horizontal line peak at theta=-90", ok, theta(peaks(1,2)), 0);
if ok then passed = passed+1; else failed = failed+1; end
...
// ------------------------------------------------------------
// TC-06: All-zero H returns repeating first index peaks [1, 1]
// ------------------------------------------------------------
mprintf("\n--- TC-06: All-Zero H Returns Initial Index Ties ---\n");
H = zeros(100, 180);
peaks = houghpeaks(H, 5);
expected_peaks = ones(5, 2);
ok = isequal(peaks, expected_peaks);

report("All-zero accumulator returns initial index tie grid", ok, peaks, expected_peaks);
if ok then passed = passed+1; else failed = failed+1; end
