function angles = colorangle (rgb1, rgb2)
    ...
    if (size(rgb1, 1) ~= size(rgb2, 1) & (size(rgb1, 1) ~= 1 & size(rgb2, 1) ~= 1)) then
        error ("colorangle: RGB1 and RGB2 must have one or same number of colors");
    end
...
function rgb = check_rgb (rgb, name)
    if (type(rgb) > 8 | ~isreal(rgb)) then
         error("colorangle: " + name + " must be a real numeric array");
    end
    
    if (size(rgb, "*") == 3) then
        rgb = rgb(:)';
    elseif (size(rgb, 2) ~= 3) then
        error ("colorangle: " + name + " must be a 3 element or Nx3 array");
    end
endfunction
